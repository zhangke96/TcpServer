// Copyright 2018 zhangke
#ifndef TCPSERVER_AUX_CLASS_H_
#define TCPSERVER_AUX_CLASS_H_
...
class fdwrap {
 ...
 public:
  fdwrap()
      : fdsp(new int(-1), close) {
  }  // default constructor, assign a invaild fd -1
  explicit fdwrap(int filed) : fdsp(new int(filed), close) {}
  ssize_t read(char *buf, size_t size) { return ::read(*fdsp, buf, size); }
  ssize_t write(char *buf, size_t size) { return ::write(*fdsp, buf, size); }
  int getfd() const { return *fdsp; }
  ...
  void reset(int newfd) { fdsp.reset(new int(newfd), close); }
  fdwrap(const fdwrap &rhs) : fdsp(rhs.fdsp) {}
  ...
  fdwrap(fdwrap &&rhs) noexcept : fdsp(std::move(rhs.fdsp)) {}
 ...
 private:
  ...
  static void close(int *fdp) {
    ...
    if (::close(*fdp) != 0) {
      Log(logger, Logger::LOG_ERROR,
          "error when close fd: " + std::to_string(*fdp));
    }
    delete fdp;
  }
};
...
class MutexWrap {
 public:
  MutexWrap() { pthread_mutex_init(&mutex, NULL); }
  ~MutexWrap() { pthread_mutex_destroy(&mutex); }
...
class AutoMutex {
 public:
  explicit AutoMutex(MutexWrap &amutex) : mutex(amutex) { mutex.lock(); }
